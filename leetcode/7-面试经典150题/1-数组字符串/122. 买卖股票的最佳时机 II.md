...
> **日期:** 2024-08-30、2026-02-27  
> **所用时间:** 15min

## 1. 记忆化搜索

`dfs(i, hold)` 表示在第 $i$ 天，持有股票为 $hold$ 的最大利润

状态转移：

1. 如果持有股票，则可以卖出股票，或者不卖出股票
    - 卖出股票：$dfs(i, True) = dfs(i - 1, False) + prices[i]$
    - 不卖出股票：$dfs(i, True) = dfs(i - 1, True)$
2. 如果不持有股票，则可以买入股票，或者不买入股票
    - 买入股票：$dfs(i, False) = dfs(i - 1, True) - prices[i]$
    - 不买入股票：$dfs(i, False) = dfs(i - 1, False)$
